            ...
            <Button
              ...
              onPress={async () => {
                ...
                if (!result.canceled) {
                  ...
                  const newUri = await FileManager.saveImageToAppFolder(
                    localUri
                  );
          ...
          <TextInput
            ...
            onChangeText={(text) => {
              if (paramType === "INTEGER") {
                // nur 0-9, . und , erlauben
                let sanitized = text.replace(/[^0-9.,]/g, "");

                // höchstens ein Komma
                const partsComma = sanitized.split(",");
                if (partsComma.length > 2) {
                  sanitized =
                    partsComma[0] + "," + partsComma.slice(1).join("");
                }

                // höchstens ein Punkt
                const partsDot = sanitized.split(".");
                if (partsDot.length > 2) {
                  sanitized =
                    partsDot[0] + "." + partsDot.slice(1).join("");
                }
